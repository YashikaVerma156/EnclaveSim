...
#ifdef ENCLAVE
    // addition of enclave-id into the instruction packets.
    IFETCH_BUFFER.entry[index].enclave_id = return_enclave_id(cpu, instr_pa);
#endif
...
void O3_CPU::fetch_instruction()
{
    ...
    for (uint32_t i = 0; i < IFETCH_BUFFER.SIZE; i++)
    {
...
#ifdef ENCLAVE
            trace_packet.enclave_id = return_enclave_id(cpu, trace_packet.full_addr); // addition of enclave-id parameter in trace_packet.
#endif
        ...
        if ((IFETCH_BUFFER.entry[index].translated == COMPLETED) && (IFETCH_BUFFER.entry[index].fetched == 0))
        {
...
#ifdef ENCLAVE
            fetch_packet.enclave_id = return_enclave_id(cpu, fetch_packet.full_addr);
#endif
            ...
            if (hit_way != -1)
            {
                prefetch_hit = L1I.block[L1I.get_set(fetch_packet.address, cpu)][hit_way].prefetch;
            }
...
#ifdef ENCLAVE
        pf_packet.enclave_id = return_enclave_id(cpu, pf_pa);
#endif
...
#ifdef ENCLAVE
    data_packet.enclave_id = return_enclave_id(cpu, data_packet.full_addr);
#endif
