...
// fill the data into the cache, from the MSHR
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
    {
...
#ifdef ENCLAVE
        assert_enclave_id(&(MSHR.entry[mshr_index]));
#endif
...
#ifdef ENCLAVE
            update_enclave_hit_miss_stats(&(MSHR.entry[mshr_index]), fill_cpu, false);
#endif
		...
		*/
            ...
            // update the next_fill index of the MSHR.
            update_fill_cycle();
        }
    }
}
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0))
    {
...
#ifdef ENCLAVE
        assert_enclave_id(&(WQ.entry[index]));
#endif
...
#ifdef ENCLAVE
            update_enclave_hit_miss_stats(&(WQ.entry[index]), writeback_cpu, true);
#endif
        ...
        { // writeback miss (or RFO miss for L1D)
            ...
            { // RFO miss
                ...
                else if (mshr_index == -1 && is_mshr_vacant)
                { // this is a new miss
...
#ifdef ENCLAVE
                            writeback_packet.enclave_id = block[set][way].enclave_id;
#endif
                            lower_level->add_wq(&writeback_packet);
                        }
                    }
...
#ifdef ENCLAVE
                    update_enclave_hit_miss_stats(&(WQ.entry[index]), writeback_cpu, false);
#endif
...
#ifdef ENCLAVE
            assert_enclave_id(&(RQ.entry[index]));
#endif
...
#ifdef ENCLAVE
                update_enclave_hit_miss_stats(&(RQ.entry[index]), read_cpu, true);
#endif
...
#ifdef ENCLAVE
                                RQ.entry[index].enclave_id = return_enclave_id(read_cpu, pa);
...
#ifdef ENCLAVE
                update_enclave_hit_miss_stats(&(PQ.entry[index]), prefetch_cpu, true);
#endif
                    ...
                    if (lower_level)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (PQ.entry[index].fill_level <= fill_level)
                                    add_mshr(&PQ.entry[index]);
                                if (PQ.entry[index].enclave_id == -1)
                                {
                                    assert(0 && "add_pq assert failed!");
                                }
...
#ifdef ENCLAVE
    block[set][way].enclave_id = packet->enclave_id;
#endif
...
int CACHE::prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int pf_fill_level, uint32_t prefetch_metadata)
{
    ...
    if (PQ.occupancy < PQ.SIZE)
    {
        if ((base_addr >> LOG2_PAGE_SIZE) == (pf_addr >> LOG2_PAGE_SIZE))
        {
            ...
            pf_packet.enclave_id = return_enclave_id(cpu, pf_addr);
            add_pq(&pf_packet);
...
void CACHE::assert_enclave_id(PACKET *packet)
{
    ...
    {
        uint64_t page_number = packet->full_addr >> LOG2_PAGE_SIZE;
        if (packet->enclave_id == -1)
            assert(0 && "Enclave-ID is not assigned in the packet.");
        if (page_number > 0 and page_number <= ENCLAVE_PAGES)
            assert(packet->enclave_id != NUM_CPUS && "Enclave-ID is not assigned properly.");
        else
            assert(packet->enclave_id == NUM_CPUS && "Enclave-ID is not assigned properly.");
    }
}
...
void CACHE::update_enclave_hit_miss_stats(PACKET *packet, uint32_t cpu, bool is_hit)
{
    ...
    {
        if (is_hit)
            sim_enclave_hit[cpu][packet->type]++;
        else
            sim_enclave_miss[cpu][packet->type]++;
        sim_enclave_access[cpu][packet->type]++;
    }
}
